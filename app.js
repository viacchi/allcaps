...
if (agreeCheck && agreeBtn) {
  // Enable button when checkbox is checked
  agreeCheck.addEventListener("change", () => {
    agreeBtn.disabled = !agreeCheck.checked;
  });

  // When clicked, show alert and close modal
  agreeBtn.addEventListener("click", () => {
    alert("Thank you! You have agreed to the Terms & Conditions.");
    closeTerms(); // reuses your existing closeTerms() function
  });
}

const topBtn = document.getElementById("topBtn");
const termsContent = document.getElementById("termsContent"); // scrollable content div

if (topBtn && termsContent) {
  topBtn.addEventListener("click", () => {
    termsContent.scrollTo({ top: 0, behavior: "smooth" });
  });
}
